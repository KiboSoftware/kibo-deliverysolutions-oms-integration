...
// import { ShipmentApi, ShipmentNotesApi } from "your-shipment-api-library";
// import { EntityModelOfShipment } from "your-shipment-models";
// import { Configuration } from "your-configuration-library";

import { EntityModelOfShipment, ShipmentApi, ShipmentNotesApi } from "@kibocommerce/rest-sdk/clients/Fulfillment";
import { KiboShipmentService } from "../../services/kiboShipmentService";
...
describe("KiboShipmentService", () => {
  ...
  describe("getShipmentById", () => {
    test("should return shipment by ID", async () => {
      ...
      const expectedShipment: EntityModelOfShipment = { /* mocked shipment object */ };
  ...
  describe("cancel", () => {
    test("should cancel shipment", async () => {
      const shipmentNumber = 123;
      const expectedShipment: EntityModelOfShipment = { /* mocked shipment object */ };
      shipmentApi.cancelShipment = jest.fn().mockResolvedValue(expectedShipment);
  ...
  describe("sendToCustomerCare", () => {
    test("should send shipment to customer care", async () => {
      const shipmentNumber = 123;
      const expectedShipment: EntityModelOfShipment = { /* mocked shipment object */ };
      shipmentApi.customerCareShipment = jest.fn().mockResolvedValue(expectedShipment);

      const result = await kiboShipmentService.sendToCustomerCare(shipmentNumber);
  ...
  describe("execute", () => {
    test("should execute shipment task", async () => {
      ...
      const result = await kiboShipmentService.execute(shipmentNumber, taskName);
